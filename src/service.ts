...
export const authService = (
  ...
  const handleLogin = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
      ...
      logging.debug(settings.authEnv, '[handleLogin] Calling onSignup callback');

      // Call the login callback
      const userAuth = await settings.onLogin(req, res);

      logging.debug(settings.authEnv, '[handleLogin] onSignup callback finished successfully');
